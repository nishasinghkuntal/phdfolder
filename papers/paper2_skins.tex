...
% NOTE (24 Sep 2026): all skin-only numbers below are from the recomputed
% grid (skin_grid_fixed_*.csv, corrected convergence test). Numbers that
% combine cooling with a skin (Table constraints cooling rows, tension
% costs 0.78/2.07) must still be regenerated with joint_likelihood.py
% and cooling_chi2_final.csv -- marked CHECK.
\begin{abstract}
...
$(\Esym,\Lsym)$, the solver reproduces neutron skins to $0.2$--$1.7$
milli-fermi and charge radii to
...
the models predict nearly equal skins for the two nuclei, $0.174$ and
$0.173$~fm at the joint best fit, whereas the measurements call for $0.283$ and
...
Convergence is not judged on the skin alone. In the slow early sweeps of the
self-consistency loop both radii can drift outward together by $\sim0.1$~fm
while their difference hardly changes, so a test on the skin can stop a
calculation that has not converged; such a solution is over-bound and too
compact. We therefore require the skin \emph{and} the proton radius to be
stable to $2\times10^{-4}$~fm over three consecutive sweeps, after at least
twenty sweeps.
% With this test all 148 grid points were recomputed; individual skins moved
% by up to 0.007 fm (rms 0.004 fm for Pb-208, 0.003 fm for Ca-48).
...
is $0.2$--$1.7$ milli-fermi, between forty and three hundred times finer than
the PREX-2 uncertainty. Charge radii agree to $0.023$~fm. Binding energies come
out $0.09$--$0.24$~MeV per nucleon below the published values, a systematic
under-binding that does not affect the skins, which are differences of radii.
% CHECK: the under-binding (also 0.16 MeV/A for NL3 Pb-208) is being
% investigated; before the convergence fix it was masked by calculations
% that stopped early and came out over-bound.

As an independent benchmark we also run two parametrizations that play no
role in the scan. For NL3 \citep{NL3} we obtain $\DRnp(^{208}\mathrm{Pb})=0.277$~fm
against the published $0.280$~fm, and for FSUGold \citep{ToddRutel2005}
$0.207$~fm against $0.207$~fm, with charge radii within $0.03$~fm.
...
$0.17$~MeV per nucleon of experiment, the largest departure being $^{48}$Ca at
$-0.169$~MeV per nucleon. Second, the
...
$0.0021$--$0.0031$~fm/MeV at $\Lsym = 40$~MeV to
$0.0005$--$0.0007$~fm/MeV at $\Lsym = 100$~MeV as the skin saturates (at
$\Esym=32.5$~MeV), passing through the published model-averaged correlation
slope of $0.00147$~fm/MeV \cite{RocaMaza2011} between $\Lsym \simeq 64$ and
$81$~MeV depending on the parametrization; at $\Lsym = 60$~MeV, the centre of
the cooling band, it is $0.0016$--$0.0023$~fm/MeV. We
...
leaves root-mean-square residuals of $0.014$--$0.015$~fm for $^{208}$Pb,
half the PREX-2 uncertainty. With the quadratic term the residuals fall to
$0.0037$--$0.0072$~fm, which we add in quadrature to the
...
predictions are $\DRnp(^{208}\mathrm{Pb}) \simeq 0.174$~fm and
$\DRnp(^{48}\mathrm{Ca}) \simeq 0.173$~fm: nearly equal. The measurements call
for $0.283$ and $0.121$~fm. The joint $\chi^2$ at that point is $4.46$. With two
...
% CHECK: recompute these two costs (and the cooling rows of the constraints
% table) with joint_likelihood.py on skin_grid_fixed_*.csv and the final
% cooling chi2; skin-only rows are unchanged by the recomputation.
...
reproduced exactly: its minimum $\chi^2$ alone is $1.06$ anywhere on the grid.
