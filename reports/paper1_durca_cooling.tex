...
\date{Status as of 24 September 2026 (updated)}
...
\end{equation}
and used two covariant density functionals, GM1 (non-linear) and DDMEX (density dependent). Keeping all other couplings as published, they retuned only the $\rho$ meson coupling so that $\Esym(n_0)=38.1$ MeV (and $\Esym\approx28$ MeV at 0.1 fm$^{-3}$, to keep finite nuclei reasonable), and looked at $\Lsym=70$, 85 and 106 MeV. Their main points are:
\begin{itemize}
\item the proton fraction hardly changes with $\Esym$ inside the PREX-2 range, so they fix $\Esym=38.1$ MeV and vary only $\Lsym$ (their Fig.~1);
\item with $\Lsym=85$ MeV DUrca opens above $M_{\rm DU}\approx0.978\,\Msun$ for GM1 and $1.125\,\Msun$ for DDMEX, at $n_{\rm DU}\approx0.289$ and 0.284 fm$^{-3}$ (their Fig.~3 and Table~I); at $\Lsym\approx106$ MeV already at 0.8--0.9 $\Msun$;
\item they run NSCool with the same HZD--NV crust that I use, iron envelopes, and stars of 1.2, 1.43 and 2.0 $\Msun$, and find that with superfluid suppression 13 isolated stars are explained, with Cas~A needing a weaker neutron gap.
\end{itemize}
In short, they show that the PREX-2 values make fast cooling possible in canonical mass stars and that this is compatible with the data.

My work goes one step further, and also uses GM1 as one of its models, so the two can be compared directly (Sec.~\ref{sec:mdu}). Instead of fixing $\Esym$ and $\Lsym$ at the PREX-2 values, I scan a wide equispaced grid in $(\Esym,\Lsym)$, compute cooling curves for every point, and use the observed stars to find which part of the grid is preferred. In this way the cooling data \emph{constrain} the symmetry energy instead of only testing one value.
...
\end{equation}
where $W_0$ solves $W_0/C_\omega^2+\zeta W_0^3/6=n_0$ and $(dW/dn)_0=[1/C_\omega^2+\zeta W_0^2/2]^{-1}$. I took these relations from the parameter note \texttt{RMFparameters.pdf} (Eqs.~19--23), which is the isovector prescription of Thapa and Sinha \cite{ThapaSinha2022} that Sarkar et al.\ also follow \cite{Sarkar2023} \fillin{confirm from the title page of the PDF}, and generalised them to $\zeta\neq0$; for $\zeta=0$ they reduce to the original form. Table~\ref{tab:J0L0} gives $J_0$ and $L_0$ as my code computes them.
...
NL3$\omega\rho$ ($\Esym=31.7$, $\Lsym=55.5$)$^a$ & 2.756 & 2.75 & \cite{Fortin2016}\\
FSUGarnet ($\Esym=30.95$, $\Lsym=51.04$)$^a$ & 2.073 & 2.07 & \cite{FSUGarnet}\\
IOPB-I ($\Esym=33.30$, $\Lsym=63.58$)$^a$ & 2.120 & 2.15 & \cite{IOPBI}\\
BigApple ($\Esym=31.32$, $\Lsym=39.80$)$^a$ & 2.593 & 2.60 & \cite{BigApple}\\
\bottomrule
\end{tabular}
\end{adjustbox}

\smallskip
{\footnotesize $^a$ Computed with \texttt{durca\_threshold\_mass.py}: same EoS code, but a short Python TOV using the RMF core table down to 0.03 fm$^{-3}$ plus a $\Gamma=4/3$ outer layer instead of the NSCool crust. It gives 2.361 $\Msun$ for GM1 (full pipeline 2.360), so masses are reliable to about 0.01 $\Msun$. Radii from it are 0.2--0.4 km too small without the real crust, so I do not quote them. The $\Esym$, $\Lsym$ of the three modern sets are the published values in Table~2 of Ref.~\cite{Das2021}.}
\end{table}
For GM, setting $\Lsym=L_0+3(\Esym-J_0)$ makes $\Lambda_{\omega\rho}$ exactly zero, and with $\Esym=32.5$ MeV this is the original GM isovector sector. The code returns $C_\rho^2=4.414$ fm$^2$ (GM1) and 4.794 fm$^2$ (GM2, GM3), against 4.411 and 4.791 in their Table~II, and the maximum masses agree within 0.2--0.5\%. The NL3$\omega\rho$ test is the one with $\Lambda_{\omega\rho}\ne0$: I do not copy its $g_\rho$ and $\Lambda$ but let the code derive them from the published $\Esym$ and $\Lsym$; it returns $\Lambda_{\omega\rho}=0.0297$, against about 0.03 in the literature, and $M_{\max}=2.756\,\Msun$ against 2.75. FSUGarnet and BigApple agree within 0.3\%. IOPB-I comes out 1.4\% low (2.120 against 2.149). I traced this to $\zeta$: the code derived $\zeta=0.0191$ for IOPB-I, while the published E-RMF value $\zeta_0=3.103$ with $g_\omega/4\pi=1.062$ \cite{IOPBI} corresponds to $\zeta=\zeta_0/g_\omega^2=0.0174$ in this Lagrangian. With the published $\zeta$ (and $b$, $c$, $C_\sigma^2$, $C_\omega^2$ refitted exactly to the published saturation point) the code gives $M_{\max}=2.149\,\Msun$, exactly the published value; the same test with the published $\zeta=0.0235$ (FSUGarnet) and $0.0007$ (BigApple) gives 2.067 and 2.604 $\Msun$ (published 2.07 and 2.60). So the solver, TOV and Lagrangian are right, and the small differences come only from the $\zeta$ values used in the scan. Effect on the analysis: $M_{\max}$ of IOPB-I rises by 0.03 $\Msun$ and $M_{\rm DU}$ changes by at most 0.02 $\Msun$ (Sec.~\ref{sec:checks}). \fillin{confirm the three published $\zeta$ values from the parameter tables of Refs.~\cite{FSUGarnet,IOPBI,BigApple}; decide whether to rerun the scan with them}
...
\noindent\textbf{Source of these numbers.} The paper~1 draft attributes the 67/150 eV two-component fit to Zavlin et al.\ 1999 \cite{Zavlin1999}, but that paper reports a single hydrogen/helium atmosphere fit with $T^\infty_{\rm eff}=(1.6$--$1.9)\times10^6$ K. My analysis script (\texttt{full\_multimass\_5mode.py}) records the real sources: the two-component (2NSA) fit is the one listed in the Potekhin et al.\ 2020 catalogue \cite{Potekhin2020}, and the full-surface magnetised hydrogen (nsmax) value $1.18\pm0.05$ MK is noted there as ``Shibanov et al.\ (2016)''. The only Shibanov et al.\ 2016 paper I can find (ApJ 831, 112) is about the neutron star Calvera, so this citation must be checked in Table~1 of Potekhin et al.\ 2020 before the paper is submitted. \fillin{find the exact paper for the 2NSA fit and the 1.18 MK value in the Potekhin 2020 table; replace the Zavlin 1999 citation in the paper~1 draft}

The same script notes that other acceptable fits span 0.79--1.34 MK ($\log T=5.90$--6.13), so $\sigma=0.05$ dex is probably too small for this star; $\sigma\approx0.12$ would cover the spread. This is a systematic worth testing, since RX~J0822 had such a large weight before the correction.
...
\subsection{Direct Urca threshold masses inside the allowed region}\label{sec:mdu}
For each EoS the threshold density $n_{\rm DU}$ comes from the EoS code, and $M_{\rm DU}$ is the mass of the star whose central density equals $n_{\rm DU}$. I read it off the pipeline $M$--$R$ curves (\texttt{MRcurve\_*\_CAT.dat} from \texttt{TOVprofile2\_automated.f90} with the NSCool crust): the central energy density at $n_{\rm DU}$ is taken from the EoS table and $M$ is interpolated on the stable branch. Curves were available for 683 of the 864 EoS and for 152 of the EoS in the box $\Lsym=50$--75 MeV, $\Esym=28$--36 MeV. As a cross-check, \texttt{durca\_threshold\_mass.py} (a short Python TOV without the real crust) gives the same $M_{\rm DU}$ within 0.015 $\Msun$ (mean difference 0.0003 $\Msun$ over 624 EoS). Data: \texttt{reports/data/durca\_threshold\_pipeline\_MR.dat}.

\begin{table}[h]
\centering
\caption{Direct Urca threshold mass in the 68\% box, from the pipeline $M$--$R$ curves. ``none'' means DUrca never opens in a stable star.}\label{tab:mdu}
\begin{tabular}{lccc}
\toprule
Model & EoS & $M_{\rm DU}\le1.4\,\Msun$ & $M_{\rm DU}$ range (median) of the rest\\
\midrule
GM1 & 24 & 0 & 1.90--2.31 (2.21); 15 none\\
GM2 & 21 & 0 & 1.64; 20 none\\
FSUGarnet & 35 & 7 & 1.15--1.99 (1.64)\\
IOPB-I & 36 & 7 & 1.11--2.06 (1.64)\\
BigApple & 36 & 1 & 1.40--2.60 (2.17)\\
\midrule
all & 152 & 15 (9.9\%) & 1.11--2.60 (1.77); 35 none\\
\bottomrule
\end{tabular}
\end{table}

At $\Esym=32.5$ MeV, $M_{\rm DU}$ falls smoothly with $\Lsym$: for FSUGarnet from 1.78 $\Msun$ at $\Lsym=50$ to 1.37 at 75, for IOPB-I from 1.80 to 1.33, for BigApple from 2.40 to 1.69; for GM1 DUrca appears only at $\Lsym\ge70$ (2.26 and 1.99 $\Msun$) and for GM2 not at all in this box. The single best EoS (GM2, 32.5, 80) has $M_{\rm DU}=1.47\,\Msun$ and $M_{\max}=2.006\,\Msun$. So the picture behind the fit is: 1.4 $\Msun$ stars without DUrca, and the two cold stars that pick 1.8 $\Msun$ are above threshold.

\paragraph{Cross-check with Sarkar, Thapa and Sinha.} For GM1 at their point $\Esym=38.1$, $\Lsym=85$ MeV my codes give $n_{\rm DU}=0.305$ fm$^{-3}$ and $M_{\rm DU}=1.05\,\Msun$; they report about 0.289 fm$^{-3}$ and $0.978\,\Msun$ \cite{Sarkar2023}. The agreement is at the 5\% level, and the difference is expected because they also fix $\Esym(0.1~{\rm fm}^{-3})\approx28$ MeV and use a different crust in the TOV. At $\Lsym=105$ MeV I get $0.78\,\Msun$, again close to their 0.8--0.9 $\Msun$.
...
\item $\Lsym=50$--75 MeV is lower than the PREX-2 inference $106\pm37$ MeV \cite{Reed2021} used in Ref.~\cite{Sarkar2023}, and agrees with nuclear structure results: 35--70 MeV from isobaric analog states \cite{DanielewiczLee2014}, 40.5--61.9 MeV from the combination of Lattimer and Lim \cite{LattimerLim2013}, about $60\pm25$ MeV in Roca-Maza et al.\ \cite{RocaMaza2011}, and with the chiral EFT \cite{Drischler2020} and astrophysical \cite{Essick2021} ranges. I see this as a check of the cooling method more than an improvement in precision. The new analysis of Montefusco et al.\ \cite{Montefusco2026} finds a lower $\Lsym=29\pm8$ MeV from nuclear data; this tension is discussed in Sec.~\ref{sec:newpaper}.
...
\end{adjustbox}
\end{center}
The small GM differences come from the 4-digit rounding in their Table~II. For BigApple $K$ comes out 1.4\% higher than published. Reason: for the $\zeta\neq0$ sets $b$, $c$, $\zeta$ were derived from $P(n_0)=0$, the $\sigma$ equation and the energy density with $C_\sigma^2$, $C_\omega^2$ taken from the published couplings, so $K$ was not imposed. Refitting the isoscalar sector exactly to $(n_0,E/A,K,M^*/M)$ at fixed $\zeta$ (a $\zeta\neq0$ version of \texttt{isoscalar\_fit.py}) gives $K=227.1$ MeV exactly, and changes $M_{\max}$ by $\le0.001\,\Msun$ and $M_{\rm DU}$ by $\le0.005\,\Msun$ at the published $\Esym$, $\Lsym$ of all three sets. So this difference has no effect on the results.
...
\item \textbf{Isoscalar calibration}: exact refit of the $\zeta\neq0$ sets to $(n_0,E/A,K,M^*/M)$ changes $M_{\max}$ by $\le0.001\,\Msun$; with the published $\zeta$ the code reproduces the published $M_{\max}$ of IOPB-I (2.149 $\Msun$), FSUGarnet and BigApple within 0.2\%.
\item \textbf{Onset masses}: read from the pipeline $M$--$R$ curves and cross-checked with an independent Python TOV (agreement within 0.015 $\Msun$ over 624 EoS).
\item \texttt{Verification/verify\_papers.py} recomputes every number of the paper 1 draft from the data files (grid counts, best fit, intervals, radius, correlation) and rebuilds the paper; the last log shows all checks passing (e.g.\ best $\chi^2/N=1.3967$, $\chi^2/{\rm dof}=1.5963$, GM2 at 32.5/80, $R_{1.4}=13.20\pm0.39$ km). \texttt{Verification/check\_dois.py} checks the bibliography against Crossref.
\end{enumerate}
...
\paragraph{What they do.} They build the EoS with a flexible, asymptotically causal nucleonic metamodel and do a Bayesian analysis. The laboratory input is (i) the full correlated posterior of nuclear matter parameters from a large set of nuclear structure data (binding energies and radii of 150 spherical nuclei, giant resonances, dipole polarisability, and the PREX/CREX parity-violating asymmetries), called NSD, and (ii) the INDRA--FAZIA isospin transport data from Ni+Ni collisions at 32 MeV/nucleon, used as a density-weighted likelihood along each sampled $S(n)$ curve, called IF. They add a chiral EFT prior and an astrophysical likelihood (heavy pulsars, GW170817, and NICER for PSR J0030, J0437, J0740 and J0614). The DUrca condition they use is the same momentum condition as mine, in $npe\mu$ matter, and $M_{\rm DU}$ is the lightest stable star in which it is satisfied somewhere in the core.

\paragraph{Their numbers (final ensemble FN+astro).}
\begin{itemize}
\item $P(M_{\rm DU}\le1.4\,\Msun)$ drops from 17.5\% (chiral + astro) to 1.2\% (+IF) and 0.26\% (+IF+NSD). About 3.3\% of the weight has no DUrca at all.
\item $M_{\rm DU}=1.89^{+0.24}_{-0.21}\,\Msun$, $n_{\rm DU}=0.50^{+0.13}_{-0.07}$ fm$^{-3}$, proton fraction at threshold $x_{p,\rm DU}\simeq0.137$.
\item Nuclear parameters: $\Esym=29.4\pm0.7$ MeV, $\Lsym=29^{+8}_{-8}$ MeV (FN posterior); with the INDRA--FAZIA data alone $\Esym=28.9\pm0.9$ and $\Lsym=41^{+14}_{-15}$ MeV.
\item Stars: $M_{\rm TOV}=2.26^{+0.13}_{-0.13}\,\Msun$, $R_{1.4}=12.10^{+0.28}_{-0.39}$ km.
\item Caveats they state: parabolic isospin dependence assumed; hyperonic DUrca not excluded; massive stars remain uncertain.
\end{itemize}

\paragraph{Comparison with my results.}
\begin{center}
\begin{tabular}{lcc}
\toprule
 & This work (cooling) & arXiv:2609.24940 (FN+astro)\\
\midrule
$\Esym$ (MeV) & 28.0--36.0 (68\%) & $29.4\pm0.7$\\
$\Lsym$ (MeV) & 50--75 (68\%), 35--80 (95\%) & $29^{+8}_{-8}$ (IF alone $41^{+14}_{-15}$)\\
$M_{\rm DU}$ ($\Msun$) & median 1.77 in the 68\% box; best fit 1.47 & $1.89^{+0.24}_{-0.21}$\\
$P(M_{\rm DU}\le1.4\,\Msun)$ & 9.9\% of EoS in the 68\% box$^*$ & 0.26\%\\
$R_{1.4}$ (km) & 12.96--13.63 & $12.10^{+0.28}_{-0.39}$\\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize $^*$ A fraction of grid points, not a posterior probability. The like-for-like number is the cooling-likelihood weighted $P(M_{\rm DU}\le1.4\,\Msun)$, which needs \texttt{Data/cooling\_chi2\_final.csv}. \fillin{compute}}
\end{center}

\paragraph{My assessment: partly a help, partly a real tension.}
\begin{enumerate}
\item \textbf{Where it helps.} The physical picture is the same: DUrca should not work in $\le1.4\,\Msun$ stars but can work in heavier ones. Their median $M_{\rm DU}=1.89\,\Msun$ and my median 1.77 $\Msun$ (best fit 1.47, with the two cold stars assigned $1.8\,\Msun$) agree well. Their $\Esym$ lies inside my 68\% interval. And both results are well below the PREX-2 based values used by Sarkar, Thapa and Sinha \cite{Sarkar2023}, where DUrca opens near $1\,\Msun$. So it supports our message that cooling does not need the very stiff PREX-2 symmetry energy.
\item \textbf{Where it is a problem.} Their $\Lsym=29\pm8$ MeV is clearly lower than my 50--75 MeV: the 68\% intervals do not overlap and only touch at my 95\% edge (35 MeV). Their radius 12.1 km is also about 1 km smaller than my 13.2 km (my radius is about $1\sigma$ above NICER, theirs is on it). A referee will certainly ask about this.
\item \textbf{Why the difference may be partly method.} (a) My grid cannot go below $\Lsym=30$ MeV at all, and FSUGarnet, IOPB-I and BigApple cannot go below 50, 45 and 35 MeV (no real $g_\rho$), so the low-$\Lsym$ region they prefer is barely covered by my model family. (b) In RMF models with $\omega$--$\rho$ mixing $\Lsym$ and the high-density $S(n)$ are tied together, while their metamodel lets them move separately. (c) I fix the isoscalar sector to five published sets, while they sample it. (d) The two data sets are different: cooling probes 2--3 $n_0$, their laboratory data mostly $\le n_0$. Their INDRA--FAZIA-only value $41^{+14}_{-15}$ MeV, which is less driven by nuclear structure, does overlap with my interval.
\item \textbf{Not the same work.} They use no cooling data. So our paper is still new; but we can no longer write that $\Lsym\approx50$--75 MeV is simply ``consistent with nuclear physics'' without discussing this paper.
\end{enumerate}

\paragraph{What has been added to the paper~1 draft.} The first two points below are now in \texttt{papers/paper1\_cooling.tex} (Sec.~\ref{sec:draft}); the third is still open.
\begin{itemize}
\item Citation of the paper, the $M_{\rm DU}$ results and the statement that the DUrca picture agrees. \emph{Done.}
\item An honest discussion of the $\Lsym$ tension, with the reasons (a)--(d). \emph{Done.}
\item A test that extends the grid to lower $\Lsym$ where the models allow it (GM1, GM2 below 30 MeV), or adds one parametrisation that reaches $\Lsym\approx20$--30 MeV, to see whether the cooling $\chi^2$ really rises there. \fillin{open}
\end{itemize}

%=====================================================================
\section{Changes made to the paper~1 draft}\label{sec:draft}
%=====================================================================
The draft \texttt{papers/paper1\_cooling.tex} (MNRAS version) has been corrected as follows. The original is kept in the repository history, so every change can be seen line by line.
\begin{enumerate}
\item \textbf{RX J0822$-$4300 (Sec.~``A correction to an adopted surface temperature'').} The two-component fit is now attributed to the Potekhin et al.\ 2020 compilation instead of Zavlin et al.\ 1999, and a sentence gives the spread of acceptable whole-surface fits (0.79--1.34 MK). Two \texttt{\% CHECK} comments mark where the original references must still be added.
\item \textbf{New subsection ``Direct Urca threshold masses''} with the numbers of Sec.~\ref{sec:mdu} (median 1.77 $\Msun$, 9.9\% at or below 1.4 $\Msun$, best fit 1.47 $\Msun$).
\item \textbf{New paragraph on Montefusco et al.\ \cite{Montefusco2026}} in the discussion of the $\Lsym$ interval: agreement on $M_{\rm DU}$ and $\Esym$, tension in $\Lsym$, and why part of it comes from the model family.
\item \textbf{Two new systematics paragraphs:} the isoscalar calibration of the $\zeta\ne0$ sets (Sec.~\ref{sec:tov}, Sec.~\ref{sec:checks}) and the RX~J0822 temperature spread.
\item \textbf{New bibliography entry} \texttt{Montefusco2026} in \texttt{papers/refs\_additions.bib}.
\end{enumerate}
\noindent\textbf{Important:} the header of the MNRAS file says it is generated from the REVTeX source by \texttt{to\_mnras.py}. The same edits must be made in the REVTeX source, or they will be lost the next time the converter runs.

\noindent\textbf{Still to do before submission} (all marked \texttt{\% CHECK} in the draft):
\begin{itemize}
\item the original references for the RX~J0822 two-component fit and for the 1.18 MK value (Table~1 of Potekhin et al.\ 2020);
\item the posterior-weighted $P(M_{\rm DU}\le1.4\,\Msun)$ from \texttt{Data/cooling\_chi2\_final.csv};
\item confirmation of the published $\zeta$ values of FSUGarnet, IOPB-I and BigApple;
\item a rerun with $\sigma=0.12$ dex for RX~J0822, to quote its effect.
\end{itemize}
...
\bibitem{ThapaSinha2022} V.~B.~Thapa and M.~Sinha, \emph{Direct URCA process in light of PREX-2}, \arxiv{2203.02272} (cited as this arXiv preprint in Ref.~\cite{Sarkar2023}).
...
\bibitem{FrimanMaxwell1979} B.~L.~Friman and O.~V.~Maxwell, Astrophys.\ J.\ \textbf{232}, 541 (1979), \ads{1979ApJ...232..541F}.
...
\bibitem{Alpar1984} M.~A.~Alpar, P.~W.~Anderson, D.~Pines and J.~Shaham, Astrophys.\ J.\ \textbf{276}, 325 (1984), \ads{1984ApJ...276..325A}.
...
\bibitem{Zavlin1999} V.~E.~Zavlin, J.~Tr\"umper and G.~G.~Pavlov, Astrophys.\ J.\ \textbf{525}, 959 (1999), \ads{1999ApJ...525..959Z}.
\bibitem{HuiBecker2006} C.~Y.~Hui and W.~Becker, \emph{X-ray observations of RX J0822$-$4300 and Puppis-A}, Astron.\ Astrophys.\ (2006), \arxiv{astro-ph/0508655}.
...
\bibitem{Das2021} H.~C.~Das, A.~Kumar, B.~Kumar, S.~K.~Biswal and S.~K.~Patra, Int.\ J.\ Mod.\ Phys.\ E \textbf{30}, 2150088 (2021), \doi{10.1142/S0218301321500889}, \arxiv{2009.10690}.
