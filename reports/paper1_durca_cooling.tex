...
\paragraph{What I suggest to add to paper 1.}
\begin{itemize}
\item Cite the paper, show the $M_{\rm DU}$ table above, and state that the DUrca picture agrees.
\item Discuss the $\Lsym$ tension honestly, with the reasons (a)--(d).
\item As a test, extend the grid to lower $\Lsym$ where the models allow it (GM1, GM2 below 30 MeV), or add one parametrisation that reaches $\Lsym\approx20$--30 MeV, and see if the cooling $\chi^2$ really rises there.
\end{itemize}
