...
\end{table}
For GM, setting $\Lsym=L_0+3(\Esym-J_0)$ makes $\Lambda_{\omega\rho}$ exactly zero, and with $\Esym=32.5$ MeV this is the original GM isovector sector. The code returns $C_\rho^2=4.414$ fm$^2$ (GM1) and 4.794 fm$^2$ (GM2, GM3), against 4.411 and 4.791 in their Table~II, and the maximum masses agree within 0.2--0.5\%. The NL3$\omega\rho$ test is the one with $\Lambda_{\omega\rho}\ne0$: I do not copy its $g_\rho$ and $\Lambda$ but let the code derive them from the published $\Esym$ and $\Lsym$; it returns $\Lambda_{\omega\rho}=0.0297$, against about 0.03 in the literature, and $M_{\max}=2.756\,\Msun$ against 2.75. FSUGarnet and BigApple agree within 0.3\%. IOPB-I comes out 1.4\% low (2.120 against 2.15); together with the 1.4\% difference in $K$ for BigApple (Sec.~\ref{sec:checks}) this suggests the isoscalar inputs of the $\zeta\neq0$ models should be checked once more against the original tables. \fillin{recheck later with the full pipeline and the NSCool crust}
...
\noindent\fillin{To check before submission:} the abstract of Zavlin, Tr\"umper and Pavlov 1999 \cite{Zavlin1999} reports a hydrogen/helium atmosphere fit with $T^\infty_{\rm eff}=(1.6$--$1.9)\times10^6$ K, not a two-component 67/150 eV fit. The two-blackbody fit may come from a later paper (for example Hui and Becker 2006 \cite{HuiBecker2006}), and the 1.18 MK full-surface value should be traced to the exact paper and table (Potekhin et al.\ 2020 \cite{Potekhin2020} lists its source). The conclusion (use the whole-surface temperature) does not change, but the citation must point to the right paper.
...
For each EoS the threshold density $n_{\rm DU}$ comes from the EoS code, and $M_{\rm DU}$ is the mass of the star whose central density equals $n_{\rm DU}$. I computed it with \texttt{durca\_threshold\_mass.py} (see the note under Table~\ref{tab:mmax}) for every EoS in the box $\Lsym=50$--75 MeV, $\Esym=28$--35.5 MeV that passes the $2\,\Msun$ and causality cuts (176 EoS). The results are in \texttt{reports/data/durca\_threshold\_68box.dat}.

\begin{table}[h]
\centering
\caption{Direct Urca threshold mass in the 68\% box. ``none'' means DUrca never opens in a stable star.}\label{tab:mdu}
\begin{tabular}{lccc}
\toprule
Model & EoS & $M_{\rm DU}\le1.4\,\Msun$ & $M_{\rm DU}$ range (median) of the rest\\
\midrule
GM1 & 36 & 0 & 1.80--2.31 (2.21); 23 none\\
GM2 & 32 & 0 & 1.45--1.64 (1.64); 30 none\\
FSUGarnet & 36 & 7 & 1.15--2.02 (1.65)\\
IOPB-I & 36 & 7 & 1.11--2.05 (1.64)\\
BigApple & 36 & 1 & 1.40--2.60 (2.17)\\
\midrule
all & 176 & 15 (9\%) & 1.11--2.60 (1.80); 53 none\\
\bottomrule
\end{tabular}
\end{table}

At $\Esym=32.5$ MeV, $M_{\rm DU}$ falls smoothly with $\Lsym$: for FSUGarnet from 1.78 $\Msun$ at $\Lsym=50$ to 1.37 at 75, for IOPB-I from 1.80 to 1.33, for BigApple from 2.40 to 1.69; for GM1 DUrca appears only at $\Lsym\ge70$ (2.26 and 1.99 $\Msun$) and for GM2 not at all in this box. The single best EoS (GM2, 32.5, 80) has $M_{\rm DU}=1.47\,\Msun$ and $M_{\max}=2.005\,\Msun$. So the picture behind the fit is: 1.4 $\Msun$ stars without DUrca, and the two cold stars that pick 1.8 $\Msun$ are above threshold.
...
$M_{\rm DU}$ ($\Msun$) & median 1.80 in the 68\% box; best fit 1.47 & $1.89^{+0.24}_{-0.21}$\\
$P(M_{\rm DU}\le1.4\,\Msun)$ & 9\% of EoS in the 68\% box & 0.26\%\\
