...
\textbf{Status (24 September, afternoon): running.} All 148 grid points and the six validation points are being recomputed with \texttt{rerun\_skin\_grid\_fixed.py} (same couplings and meson masses, 451 mesh points, new convergence test, 4 processes). With the new test a $^{208}$Pb point needs about 150--190 sweeps instead of 10--30, so the full run takes about 10 hours. Partial results are committed as \texttt{skin\_grid\_fixed\_*.csv}. The first nine $^{208}$Pb points:

\begin{center}
\small
\begin{tabular}{lcccccc}
\toprule
BigApple & \multicolumn{2}{c}{$\DRnp$ (fm)} & & \multicolumn{2}{c}{$r_{\rm ch}$ (fm)} & \\
$(\Esym,\Lsym)$ & old & new & change & old & new & $B/A$ old $\to$ new (MeV)\\
\midrule
(26.5, 58.75) & 0.1747 & 0.1683 & $-0.0063$ & 5.472 & 5.516 & 8.05 $\to$ 7.91\\
(26.5, 82.50) & 0.1999 & 0.1943 & $-0.0057$ & 5.469 & 5.505 & 8.11 $\to$ 8.00\\
(26.5, 106.25) & 0.1956 & 0.2010 & $+0.0054$ & 5.540 & 5.509 & 7.94 $\to$ 8.03\\
(26.5, 130.00) & 0.1955 & 0.2000 & $+0.0045$ & 5.550 & 5.527 & 7.95 $\to$ 8.02\\
(31.0, 35.00) & 0.1267 & 0.1226 & $-0.0041$ & 5.487 & 5.516 & 7.75 $\to$ 7.66\\
(31.0, 58.75) & 0.2029 & 0.1970 & $-0.0059$ & 5.462 & 5.507 & 7.90 $\to$ 7.75\\
(31.0, 82.50) & 0.2294 & 0.2241 & $-0.0053$ & 5.459 & 5.494 & 7.96 $\to$ 7.85\\
(31.0, 106.25) & 0.2299 & 0.2351 & $+0.0052$ & 5.528 & 5.497 & 7.80 $\to$ 7.90\\
(35.5, 35.00) & 0.1646 & 0.1705 & $+0.0059$ & 5.560 & 5.509 & 7.33 $\to$ 7.49\\
\bottomrule
\end{tabular}
\end{center}
...
What this shows:
\begin{itemize}
\item In the old grid the charge radii fell into two groups, near 5.46 and near 5.54 fm; these were calculations that stopped at different stages of the iteration. The new values all lie at 5.49--5.53 fm, close to the published BigApple value (5.495 fm), as a converged calculation should.
\item The skins move by 0.004--0.006 fm, in both directions. At fixed $\Esym$ the new skins grow smoothly with $\Lsym$ and then level off. The old ones had small dips (for example 0.1999 $\to$ 0.1956 fm from $\Lsym=82.5$ to 106.25 at $\Esym=26.5$), which are gone.
\item These shifts are small compared with the PREX-2 and CREX errors (0.071 and 0.035 fm). So the main conclusions (nearly equal Pb and Ca skins, and cooling does not discriminate between PREX-2 and CREX) are very unlikely to change. But they are as large as the old fit residuals, so the surface fits, validation table, intervals and tension costs must all be regenerated from the new grid.
\end{itemize}
\fillin{final comparison, refitted surfaces and new intervals once the run is complete}
...
\item \textbf{A note at the top of the file} that all grid-dependent numbers (validation table, residuals, intervals, tension costs, joint optimum) must be regenerated from \texttt{skin\_grid\_fixed\_*.csv}. They have \emph{not} been changed yet, because the rerun is still going.
