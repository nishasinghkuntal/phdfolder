...
\item \textbf{Convergence} is judged on the skin (stable over several sweeps), and occupations are frozen once the fields have settled, to avoid oscillation between two close levels near the Fermi surface. \emph{Correction found while writing this report:} the skin alone is not a safe test. In the slow early sweeps both radii can drift outward together by $\sim0.1$ fm while their difference hardly moves, and the old test (skin stable to $2\times10^{-4}$ fm over three sweeps) could stop there after only $\sim10$ sweeps. This happened for NL3 $^{48}$Ca ($B/A=9.20$ MeV, $r_{\rm ch}=3.38$ fm instead of 8.58 MeV and 3.46 fm) and for some scan points, e.g.\ BigApple $\Esym=35.5$, $\Lsym=106.25$ ($B/A=9.33\to8.51$ MeV, skin $0.2273\to0.2223$ fm). The test now also requires the proton radius to be stable and at least 20 sweeps. The whole skin grid is being recomputed with this test (\texttt{rerun\_skin\_grid\_fixed.py}); see Sec.~\ref{sec:rerun}.
...
\end{equation}
The $\Lsym^2$ term is needed: the skin rises fast with $\Lsym$ and then saturates. A linear fit leaves rms residuals of 0.014--0.015 fm for Pb (a fifth of the PREX-2 error), while this surface gives 0.0037--0.0072 fm (recomputed grid; before: 0.015--0.018 and 0.0034--0.0073 fm). I add this residual in quadrature to the experimental errors.
...
\midrule
BigApple & 31.32 & 39.80 & 0.1494 & 0.151 & 0.1683 & 0.170\\
FSUGarnet & 30.95 & 51.04 & 0.1636 & 0.162 & 0.1676 & 0.169\\
IOPB-I & 33.30 & 63.58 & 0.2212 & 0.221 & 0.2010 & 0.202\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
\begin{itemize}
\item Skins agree to 0.2--1.7 milli-fermi (recomputed with the corrected convergence test; before: 0.7--4.5), 40 to 300 times smaller than the PREX-2 error.
\item Charge radii agree to 0.023 fm. Binding energies are 0.09--0.24 MeV per nucleon \emph{below} the published values (also 0.16 MeV for NL3 Pb-208). Before the convergence fix this was hidden: calculations that stopped early were over-bound and by chance close to the published values. Pairing cannot explain it for doubly magic nuclei, so the energy expression (or the centre-of-mass correction) should be checked. \fillin{open issue} It does not affect the skins, which are differences of radii.
\item One model over $A=16$--208 ($^{16}$O, $^{40}$Ca, $^{48}$Ca, $^{208}$Pb) converges everywhere, with binding energies within 0.17 MeV per nucleon of experiment (O-16 $-0.05$, Ca-40 $-0.14$, Ca-48 $-0.17$, Pb-208 $-0.14$ MeV).
...
\item The local slope $d\DRnp/d\Lsym$ for $^{208}$Pb falls (at $\Esym=32.5$ MeV) from 0.0021--0.0031 fm/MeV at $\Lsym=40$ MeV to 0.0005--0.0007 fm/MeV at 100 MeV. It passes the model-averaged slope 0.00147 fm/MeV of Roca-Maza et al.\ \cite{RocaMaza2011} between $\Lsym\approx64$ and 81 MeV depending on the model, and at $\Lsym=60$ MeV it is 0.0016--0.0023 fm/MeV. I call this consistency only, because a slope inside one model and a slope across many models are not the same thing. I never fitted to that correlation.
\item \textbf{Independent benchmarks (NL3 and FSUGold).} These sets are not part of the scan and have published finite-nucleus results. The benchmark path in the code first failed (``effective mass collapsed''). I traced it to two convention errors in the benchmark inputs: NL3 writes $M^*=M+g_\sigma\sigma$ with $\sigma<0$, so the cubic coupling changes sign ($b=-g_2/(Mg_\sigma^3)=+2.055\times10^{-3}$, $c=g_3/g_\sigma^4=-2.651\times10^{-3}$), and NL3 couples the $\rho$ as $g_\rho\boldsymbol\tau\cdot\boldsymbol\rho$ without the $\tfrac12$ of this Lagrangian, so $g_\rho$ must be doubled; for FSUGold I now take $b=\kappa/2M$, $c=\lambda/6$ from the published $\kappa=1.4203$ MeV, $\lambda=0.023762$, and the starting $\omega$ field includes the $\zeta$ term. After the fix:
\begin{center}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{llccc}
\toprule
Set & Nucleus & $\DRnp$ (fm) mine / published & $r_{\rm ch}$ (fm) mine / published & $B/A$ (MeV) mine / published\\
\midrule
NL3 & $^{208}$Pb & 0.277 / 0.280 & 5.536 / 5.509 & 7.74 / 7.90\\
NL3 & $^{48}$Ca & 0.226 / $\approx$0.23--0.245 & 3.464 / 3.473 & 8.58 / 8.64\\
FSUGold & $^{208}$Pb & 0.207 / 0.207 & 5.514 / 5.521 & 7.82 / 7.89\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
NL3 $^{48}$Ca: later comparisons quote an NL3 skin of about 0.23--0.245 fm (e.g.\ in the RMF studies around CREX), which agrees with my 0.226 fm; the value 0.195 fm written in an older comment of the code is not supported and has been corrected there.
\end{itemize}
...
At the point that best fits both experiments together the predictions are $\DRnp(^{208}{\rm Pb})\simeq0.174$ fm and $\DRnp(^{48}{\rm Ca})\simeq0.173$ fm (FSUGarnet, $\Esym=31.0$, $\Lsym=55$ MeV), almost equal, while the experiments want 0.283 and 0.121 fm (Table~\ref{tab:pred}). The joint $\chi^2$ there is 4.46 (4.42 before the recomputation). With two data points and two parameters there are no degrees of freedom, so I use this number only to show how far apart the two experiments pull inside this model family, not as a goodness of fit. This is the PREX--CREX puzzle \cite{Reinhard2022} reproduced by my own code: both skins respond to the same isovector couplings in the same direction \cite{Furnstahl2002}.
...
\midrule
$^{208}$Pb & 0.174 & $0.283\pm0.071$ \cite{PREX2} & $-1.5\sigma$\\
$^{48}$Ca & 0.173 & $0.121\pm0.035$ \cite{CREX} & $+1.5\sigma$\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\subsection{Recomputation of the skin grid with the corrected convergence test}\label{sec:rerun}
\textbf{Done (24 September).} All 148 grid points and the six validation points were recomputed with \texttt{rerun\_skin\_grid\_fixed.py} (same couplings and meson masses, 451 mesh points, new convergence test). Every point converged. Results: \texttt{skin\_grid\_fixed\_Pb208.csv}, \texttt{skin\_grid\_fixed\_Ca48.csv}, \texttt{skin\_validation\_fixed.csv}.
...
\toprule
 & $^{208}$Pb & $^{48}$Ca\\
\midrule
skin change, mean / rms / max (fm) & $-0.0002$ / 0.0039 / 0.0073 & $+0.0010$ / 0.0026 / 0.0061\\
points that moved by more than 0.003 fm & 37 of 74 & 19 of 74\\
spread of $r_{\rm ch}$ within BigApple, old $\to$ new (fm) & 0.043 $\to$ 0.013 & 0.048 $\to$ 0.006\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
The old charge radii fell into two groups (calculations stopped at different stages); the new ones form one smooth set, as a converged calculation should. The skins shift in both directions with almost no net change.

\paragraph{Effect on the paper~2 results} (\texttt{joint\_likelihood.py} run on the old and on the new grid; skin-only quantities):
\begin{center}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{lcc}
\toprule
Quantity & old grid (= paper draft) & new grid\\
\midrule
PREX-2 alone, 68\% & $\Lsym$ [50, 95], $\Esym$ [35.5, 43.0] & unchanged\\
CREX alone, 68\% & $\Lsym$ [50, 70], $\Esym$ [26.5, 32.5] & unchanged\\
joint optimum & FSUGarnet 34.0/50: Pb 0.177, Ca 0.174 fm & FSUGarnet 31.0/55: Pb 0.174, Ca 0.173 fm\\
joint $\chi^2$ at optimum & 4.42 & 4.46\\
minimum $\chi^2$ of CREX alone & 1.03 & 1.06\\
quadratic fit residuals & 0.0034--0.0073 fm & 0.0037--0.0072 fm\\
validation (skins vs Das et al.) & 0.7--4.5 mfm & 0.2--1.7 mfm\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
\textbf{All conclusions of paper~2 stand}: the separate PREX-2 and CREX intervals are unchanged, the models still give nearly equal Pb and Ca skins, and the validation is better than before. The numbers that combine cooling with a skin (the cooling rows of Table~\ref{tab:constraints} and the tension costs 0.78 and 2.07) must be recomputed with the final cooling $\chi^2$ file; the run with a placeholder cooling file changes them by only 0.1--0.2 in $\chi^2$. \fillin{rerun with cooling\_chi2\_final.csv}

\noindent\textbf{New open issue:} binding energies are now 0.09--0.24 MeV per nucleon below the published values (see the validation list above). It does not touch the skins, but the paper must not claim agreement in $B/A$ to 0.08 MeV any more; the draft has been changed accordingly.

%=====================================================================
\section{Changes made to the paper~2 draft}
%=====================================================================
In \texttt{papers/paper2\_skins.tex}:
\begin{enumerate}
\item \textbf{Numerical treatment:} a paragraph describing the corrected convergence test (skin \emph{and} proton radius stable to $2\times10^{-4}$ fm over three sweeps, after at least twenty sweeps) and why the skin alone is not enough.
\item \textbf{Validation:} the NL3 and FSUGold benchmarks ($^{208}$Pb skins 0.277 and 0.207 fm against the published 0.280 and 0.207 fm).
\item \textbf{All skin-only numbers updated} from the recomputed grid: validation table and text (0.2--1.7 mfm, charge radii 0.023 fm, binding energies now stated as 0.09--0.24 MeV/A low), $A=16$--208 binding (within 0.17 MeV/A), local slopes, linear and quadratic residuals, joint optimum (0.174/0.173 fm, $\chi^2=4.46$), minimum CREX $\chi^2$ (1.06), Table of predicted skins.
\item \textbf{Still to update} (marked \texttt{\% CHECK}): the cooling rows of the constraints table and the tension costs, which need \texttt{Data/cooling\_chi2\_final.csv}.
\item The bibliography entry \texttt{NL3} is in \texttt{papers/refs\_additions.bib} (add it to \texttt{refs.bib} if it is not there).
\end{enumerate}
To finish paper~2 after the rerun I also need \texttt{Data/cooling\_chi2\_final.csv}, because the cooling + skin intervals combine both.
