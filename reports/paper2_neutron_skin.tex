...
Status (24 September): running. All 148 grid points and the six validation points are being recomputed with \texttt{rerun\_skin\_grid\_fixed.py} (same couplings and meson masses, 451 mesh points, new convergence test). With the new test a $^{208}$Pb point needs about 150 sweeps instead of 10--30, so the full run takes several hours. The first finished point, BigApple at $\Esym=26.5$, $\Lsym=58.75$, moves from 0.1747 to 0.1683 fm ($-0.006$ fm). Changes of this size are small compared with the PREX-2 and CREX errors (0.071 and 0.035 fm) but comparable with the fit residuals (0.003--0.007 fm), so the surface fits and the intervals of Table~\ref{tab:constraints} must be redone with the new grid before submission. \fillin{new validation table, fit residuals and intervals from \texttt{skin\_grid\_fixed\_*.csv}}
